...
- **Future Data Entities**: (Planned) Entities for:
    ...
    - PV-Module (PV Modules): PV-Erzeugung (PV Generation).
    - Heimspeicher (Home Storage): Batteriesysteme (Battery Systems).
    - Gasverbrauch (Gas Consumption - when available).
    - Wasserverbrauch (Water Consumption - when available).
...
4.  You will be presented with two options:
    *   **Discover devices**: The integration will scan your network for Leaflet HEMS devices. If devices are found, you will be asked to confirm and select them.
    *   **Add device manually**: If your device is not found via discovery, you can add it by providing its IP address, port (default: 2222), and nymea UUID (optional, can be fetched during setup).

### Device Confirmation

- If using discovery, a list of found devices will be shown. Select the device you want to add and confirm.
- The integration will attempt to connect and perform a handshake with the device. If successful, the device will be added to your Home Assistant instance.
...
Once the integration is set up and if further data fetching logic is implemented, you should see entities appearing for your device. The types of entities will depend on the capabilities of your specific Leaflet HEMS setup and the data exposed by its nymea instance.

**Expected Entity Categories:**

*   **Energy (Grid)**
    *   Sensor: Netzbezug (Grid Consumption) - Power being drawn from the grid.
    *   Sensor: Netzeinspeisung (Grid Feed-in) - Power being fed into the grid.
    *   Sensor: CO2-Emissionen des Netzes (Grid CO2 Emissions) - *Currently not available on Leaflet HEMS.*
*   **PV (Photovoltaics)**
    *   Sensor: PV-Erzeugung (PV Generation) - Power currently being generated by PV modules.
*   **Battery (Home Storage)**
    *   Sensor: Batterieladung (Battery Charge Level) - State of charge of the battery.
    *   Sensor: Batterieleistung (Battery Power) - Current power flow of the battery (charging/discharging).
*   **Other (Future)**
    *   Sensor: Gasverbrauch (Gas Consumption) - *Currently not available on Leaflet HEMS.*
    *   Sensor: Wasserverbrauch (Water Consumption) - *Currently not available on Leaflet HEMS.*
...
- This initial version focuses on device discovery and connection. Actual sensor data entities will be implemented in subsequent development cycles. Ensure the integration shows as "Configured" in "Devices & Services".
...
This integration is a work in progress. The following features are planned:

1.  **Data Fetching**: Implement logic to poll nymea for specific energy data points.
2.  **Entity Creation**: Create Home Assistant sensor entities for the available data.
3.  **Authentication**: Handle authentication if required by the nymea instance.
4.  **Push Updates**: Investigate push mechanisms from nymea for more real-time updates instead of polling.
